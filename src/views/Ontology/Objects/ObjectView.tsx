...
const ObjectView: FC<TabContentProps> = ({ label, values: { objectTypeId, id } }) => {
  ...
  const isReadOnly = !checkPermission(['ontology', 'edit']) || !id;
  ...
  const onSubmit = (data: Record<string, Record<string, string>>) => {
    if (objectTypeId) {
      if (isEditing) {
        ...
        dispatch(editObject(parsedData, objectTypeId, id));
      } else {
        const parsedData = parseData(data);
        dispatch(createObject(parsedData, objectTypeId));
      }
    }
  };
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        {!isReadOnly && (
          <div className="actions">
            ...
            <Button1 variant="secondary" onClick={() => navigate(-1)}>
              Cancel
            </Button1>
          </div>
        )}
