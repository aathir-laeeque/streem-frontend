...
function* postCreateScheduler({ payload }: ReturnType<typeof schedulerActions.saveScheduler>) {
  try {
    ...
    const response: ResponseObj<any> = yield call(request, 'POST', apiProcessScheduler(), {
      data,
    });
    if (response.data) {
...
function* getSchedulerList({ payload }: ReturnType<typeof schedulerActions.fetchSchedulers>) {
  try {
    const { data, pageable }: ResponseObj<any> = yield call(request, 'GET', apiProcessScheduler(), {
      params: payload,
    });
    if (data) {
